...
from Read_data import Excel_read
class BD:
    def __init__(self):
        data=Excel_read()
        self.node_list, self.link_list, self.number_of_nodes, self.number_of_links, self.origin, self.destination=data.read_links()
        self.reliability_coefficient=1
    ...
    def g_solve_RSP_BD(self):
        ...
        self.variance_limit=min_variance#(min_variance+max_variance)
    ...
    def g_output_results(self,time):
        ...
        with open("gap.csv","w") as fl:
            ...
            for i in range(len(self.global_lower_bound)):
                ...
                gap=(UB-LB)/UB
                fl.write(str(i+1)+","+str(local_LB)+","+str(LB)+","+str(UB)+","+str(gap)+"\n")
